...
void GameClassic::keyPressEvent(QKeyEvent* event)
{
    if (!state) {
        return;
    }
...
void GameClassic::paintEvent(QPaintEvent*)
{
    QPainter p(this);
    p.setBrush(Qt::red);
    ...
    case 0:
        ...
        else if(max==2048)
        {
            ...
            WinSound->play();
            QMessageBox::about(this, "游戏胜利", "score：" + QString::number(score) + " ");
            a.push_back(score);
...
GameClassic::GameClassic(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::GameClassic)
{
    ui->setupUi(this);
    ...
    //连接排行榜
    connect(ui->pushButton_2,SIGNAL(clicked(bool)),this,SLOT(rank()));

//    //连接排行榜按钮
//    connect(&wid9,&testRank::ChangeUiSlot9,this,&GameClassic::dealsottest);
    ...
    btn0->setGeometry(0,0,15,15);
    btn0->setIcon(QIcon(":/image/blue1"));
    connect(btn0, SIGNAL(clicked()), this, SLOT(ChangeTopic0()));

    btn1 = new QPushButton(this);
    toolBar->addWidget(btn1);
    btn1->setIcon(QIcon(":/image/pink2"));
    connect(btn1, SIGNAL(clicked()), this, SLOT(ChangeTopic1()));

    btn2 = new QPushButton(this);
    toolBar->addWidget(btn2);
        btn2->setIcon(QIcon(":/image/green2"));
    connect(btn2, SIGNAL(clicked()), this, SLOT(ChangeTopic2()));

    btn3 = new QPushButton(this);
    toolBar->addWidget(btn3);
        btn3->setIcon(QIcon(":/image/red2"));
    connect(btn3, SIGNAL(clicked()), this, SLOT(ChangeTopic3()));
    ...
    //排行榜界面按钮
    ui->pushButton_2->setGeometry(560, 220, 200, 60);
...
void GameClassic::ChangeTopic0()
{
    change=0;
}

void GameClassic::ChangeTopic1()
{
    change=1;
}

void GameClassic::ChangeTopic2()
{
    change=2;
}

void GameClassic::ChangeTopic3()
{
    change=3;
}
void GameClassic::PressUp() {
    //移动
    for (int i = 0; i < 4; i++) {
        for (int j = 1; j < 4; j++) {
            if (s[i][j] == 0)continue;
    ...
    if (emp == 0)
    {
        ...
        for(int l=0;l<4;l++)//先固定列
        {
            for(int h=0;h<3;h++)
            {
                ...
                if(warn==12)
                {
                    QSound *warnSound=new QSound("qrc:/music/warning.wav");
                    warnSound->play();
                }
...
void GameClassic::on_pushButton_clicked()
{
