...
GameFast::GameFast(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::GameFast)
{
    ui->setupUi(this);
...
void GameFast::on_pushButton_2_clicked()
{
    FastSound = new QSound("qrc:/music/click.wav");
    FastSound->play();
    emit ChangeUiSlot4();
}

void GameFast::onTimeout()
{
    qDebug()<<"test"<<endl;
    numFlag++;
    myRand();
...
void GameFast::keyPressEvent(QKeyEvent* event)
{
    if (!state) {
        return;
    }
...
void GameFast::paintEvent(QPaintEvent*)
{
    QPainter p(this);
    p.setBrush(Qt::red);
